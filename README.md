...
프로젝트에서 **차량 번호판 인식 알고리즘 구현**을 담당했습니다.

주요 수행 내용은 다음과 같습니다.

- 번호판 인식을 위한 Computer Vision 처리 과정 설계
- 이미지 전처리 및 번호판 ROI 추출
- OCR 기반 차량 번호 인식
- 번호판 인식 결과 후처리
- Raspberry Pi 개발 환경 구축
- 카메라 · 제어 시스템 · 데이터베이스 간 연동 테스트

단순히 OCR을 적용하는 것에 그치지 않고,
실제 차량 이미지에서 번호판 영역을 안정적으로 추출하고
인식 결과를 시스템 제어에 활용할 수 있도록 전체 인식 파이프라인을 구성했습니다.
